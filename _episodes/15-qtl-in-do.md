...
> ## Challenge 3
> Create a histogram of the LOD scores `perms`. Hint: use the `hist()` function.    
...
> > `hist(x = perms)` or `hist(x = perms, breaks = 15)`  
> > By counting number of occurrences of each LOD value (frequencies), we  
> > can approximate the 95th percentile at 7.5.  
> > `summary(perms)`
> {: .solution}
{: .challenge} 
