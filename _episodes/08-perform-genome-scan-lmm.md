...
<i>y</i> = &mu; + &beta;<sub>i</sub>X + &epsilon;

where <i>y</i> is the response variable (phenotype in this case), &mu; is the y-intercept (mean phenotype), X is the genotype, &beta;<sub>i</sub> is a regression coefficient that represents the slope of the line, and &epsilon; is the error. In the figure below, &mu; equals 110.2, and &beta;<sub>i</sub> equals -5.8. The linear model is <i>y</i> = 110.2 + -5.8X + &epsilon;. The model intersects the genotype groups AA and AB at their group means. In contrast, the null hypothesis would state that their is no difference in group means. The linear model for the null hypothesis would be <i>y</i> = 101.6 + 0X + &epsilon;. This states that the phenotype is equal to the combined mean (101.6), plus some error (&epsilon;). Genotype doesn't affect the phenotype.

![](../fig/model-equation.png)

The linear models above describe the relationship between genotype and phenotype for an individual animal. Simple linear models are inadequate for describing the relationship between genotype and phenotype in large datasets. They don't account for relatedness among individuals nor do they correct for population structure. In real populations, the true effect
of a single genetic variant is influenced by multiple variants that affect the phenotype. A  true genetic model takes into account the effect of all variants on the phenotype. 

To model the phenotypes of all individuals in the data, we can adapt the simple linear model to include all individuals.

<i>y = &mu;1 + &summa;<sub>i=1</sub><sup>M</sup>&beta;<sub>i</sub>X<sub>i</sub> + &epsilon;</i>

y represents the phenotypes of all individuals. Again,
the effect of the <i>i</i>th genotype on the phenotype is &beta;<sub>i</sub>, the mean is &mu;1, and the error is &epsilon;. Here, the number of variants is .
The true genetic model takes into account the true effect of all SNPs, including the effect of the
SNP being tested for association with a trait. When testing SNP , we are using equation (1)
the actual data is generated from 
