...
Exposure thresholds for toxicants are often determined using animal models that have limited genetic diversity, including both inbred and outbred rats and mice. These animal models fail to capture the influence of genetic diversity on toxicity responses, an important component of human responses to toxicants such as benzene. The Diversity Outbred (DO) mice reflect a level of genetic diversity similar to that of humans.
...
We loaded in three data objects. Check the Environment tab to see what was loaded.  You should see a phenotypes object called `pheno` with 598 observations (in rows) of 12 variables (in columns), an object called `map` and an object called `probs`.
