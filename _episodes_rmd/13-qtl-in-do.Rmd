...
#### Covariates    
Next, we need to create additive covariates that will be used in the mapping model. We will use study cohort and benzene concentration as a covariates in the mapping model. This study contained only male mice, but in most cases, you would include sex as an additive covariate as well.

```{r covariates}
pheno$Conc = factor(pheno$Conc, levels = c(0, 1, 10, 100))
...
qtl = scan1(genoprobs = probs, pheno = pheno[,index, drop = FALSE], kinship = K, addcovar = addcovar)
```
...
perms = scan1perm(genoprobs = probs, pheno = pheno[,index, drop = FALSE], kinship = K, addcovar = addcovar, n_perm = 100)
```
...
```{r coef}
chr = 10
coef10 = scan1blup(genoprobs = probs[,chr], pheno = pheno[,index, drop = FALSE], kinship = K[[chr]], addcovar = addcovar)
```
...
assoc = scan1snps(genoprobs = probs[,chr], map = map, pheno = pheno[,index
,drop = FALSE], kinship = K, addcovar = addcovar, query_func = query_fxn, chr = chr, start = start, end = end, keep_all_snps = TRUE)
```
